...
def get_poplar_options_per_pipeline_stage(num_ipus_per_replica, device_mapping, available_memory_proportion,
    ...
    if len(available_memory_proportion) != num_ipus_per_replica:
        if len(available_memory_proportion) == 1:
            return [
                XXXX-1.pipelining_ops.PipelineStageOptions(
            ...
            raise ValueError("Available memory proportion must be set for each of"
                             f" the {num_ipus_per_replica} XXXX-2 in the pipeline.")
    return [
        XXXX-1.pipelining_ops.PipelineStageOptions(
...
def pipeline_model(model, config, pipeline_names, pipeline_allocate_previous, num_pipeline_stages,
    ...
    :param num_pipeline_stages: An int representing the number of XXXX-2
        used in a pipeline.
    :param pipelining_mode: A str defining the type pf pipelining mode.
    :return: The model pipelined.
    """
    pipeline_stages = config.XXXX-12.pipeline_stages
    # Set pipeline stages by assigning the specific layers to specific XXXX-1 decided by user
    ...
    if pipeline_stages is not None and len(pipeline_stages) > 1:
        ...
        device_mapping = config.XXXX-12.pipeline_device_mapping
    # Equal split layers to XXXX-2 if pipeline_stages is not set
    else:
    ...
    pipeline_schedule = get_poplar_options_per_pipeline_stage(
        num_pipeline_stages, device_mapping, config.XXXX-12.available_memory_proportion, matmul_partials_type)

    model.set_pipelining_options(
        gradient_accumulation_steps_per_replica=config.XXXX-12.gradient_accumulation_factor,
        ...
        recomputation_mode=XXXX-1.pipelining_ops.RecomputationMode.RecomputeAndBackpropagateInterleaved,
        gradient_accumulation_reduction_method=XXXX-1.optimizers.GradientAccumulationReductionMethod.RUNNING_MEAN,
        gradient_accumulation_dtype=str_dtype_to_tf_dtype(config.XXXX-12.gradient_accumulation_dtype
                                                          or config.model.dtype),
        offload_weight_update_variables=config.XXXX-12.offload_optimizer_state,
        replicated_optimizer_state_sharding=config.XXXX-12.RTS)
