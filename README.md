...
An optimised hybrid GNN/Transformer model for molecular property prediction using XXXX-6 XXXX-2, trained on the [PCQM4Mv2](XXXX) dataset. The flexible hybrid model closely follows the [General, Powerful, Scalable (GPS) framework](XXXX) and combines the benefits of both message passing and attention layers for graph-structured input data training.


### Dataset

The [PCQM4Mv2](XXXX) dataset is a recently published dataset for the OGB Large Scale Challenge built to aide the development of state-of-the-art machine learning models for molecular property prediction. The task is for the quantum chemistry task of predicting the [HOMO-LUMO energy gap](XXXX) of a molecule.
...
The dataset includes four splits: train, valid, test-challenge and test-dev. The train and valid splits have true label and can be used for model development. The test-challenge split is used for the OGB-LSC PCQM4Mv2 challenge submission and test-dev for the [leaderboard](XXXX) submission.
...
The user will need to manually log in (see the quickstart guide [here](XXXX)) and configure these additional arguments:

- `--wandb_entity`
- `--wandb_projecy`

For more information please see XXXX.
